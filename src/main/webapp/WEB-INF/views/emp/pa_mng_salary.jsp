...
<div class="bdy col-span-12 lg:col-span-9 2xl:col-span-10">
	<div class="intro-y flex flex-col sm:flex-row items-center">
		<h5 class="text-lg font-medium leading-none mt-3 mr-auto">인사/행정 > 인사 관리 > 급여 관리</h5>
	</div>
	<div class="intro-y box p-5 mt-5">
		<div class="flex flex-col sm:flex-row sm:items-end xl:items-start">
		    ...
		    <input id="month" type="text" class="border-none font-medium text-xl mx-14 mt-1" style="width: 120px;">
		    <button type="button" id="next_button" class="button-next-month mt-4" style="margin-right: 240px;">
		        <svg width="11" height="16" xmlns="http://www.w3.org/2000/svg">
		            <path d="M2.748 16L0 13.333 5.333 8 0 2.667 2.748 0l7.919 8z" fill-rule="nonzero"></path>
		        </svg>
		    </button>
		    <div class="w-full sm:w-auto flex mt-4 sm:mt-0">
				<button class="btn btn-primary shadow-md mt-1" onclick="insertSalaryModal()">급여 등록</button>
			</div>
		</div>
...
$(function(){
	...
	currentMonth = currentDate.substr(0,7).replace("-", " - "); //2023 - 07
	let dbMonth = currentMonth.replace(" - ", "");
	$("#month").val(currentMonth);

	let selectedMonth = new Date($("#month").val().replace(" - ", "-"));
	// < 버튼 눌렀을때 데이터 가져오기
	...
	function monthAjax(dbMonth) {
		jQuery.ajax({
		    	url: '/emp/pa/mngsalary/month?month=' + dbMonth,
	...
	if ($("#test").length) {
	    ...
	    var table = new Tabulator("#test", {
			...
			ajaxURL: "/emp/pa/mngsalary/month",
			ajaxParams: {
			    month : dbMonth,
			},
			ajaxFiltering: false,
