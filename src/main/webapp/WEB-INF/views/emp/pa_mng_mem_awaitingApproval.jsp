...
<div class="bdy col-span-12 lg:col-span-9 2xl:col-span-10">
	<div class="intro-y flex flex-col sm:flex-row items-center mt-0">
		<h5 class="text-lg font-medium leading-none mt-0 mr-auto">계정 관리 > 입주민계정 관리 > 승인 대기 </h5>
		<div class="w-full sm:w-auto flex">
			<button class="btn btn-primary shadow-md text-base py-1.5" onclick="location.href='/emp/pa/mngmemacnt'">입주민 관리</button>
		</div>
	</div>
...
<!-- END: Body -->
<!-- BEGIN: Modal Content -->
 <div id="approvalModal" class="modal" tabindex="-1" aria-hidden="true">
     <div class="modal-dialog" >
         <div class="modal-content">
             ...
             <!-- BEGIN: Modal Body -->
             <div class="modal-body grid grid-cols-6 gap-4 gap-y-3" style="margin-left: 130px; margin-right: 100px;">
                 <div class="col-span-6 sm:col-span-6"> <label for="modal-form-1" class="form-label ml-1" style="white-space: nowrap;">승인 처리 하시겠습니까?</label></div>
             </div>
             <!-- BEGIN: Modal Footer -->
             <div class="modal-footer"> <button id="approvalModalBtn" type="button" data-tw-dismiss="modal"><span class="btn btn-primary w-20 mr-1">확인</span></button> <button type="button" data-tw-dismiss="modal" class="btn btn-outline-secondary w-20">취소</button> </div> <!-- END: Modal Footer -->
         </div>
     </div>
 </div>
 <div id="deleteModal" class="modal" tabindex="-1" aria-hidden="true">
     <div class="modal-dialog" >
         <div class="modal-content">
             ...
             <!-- BEGIN: Modal Body -->
             <div class="modal-body grid grid-cols-6 gap-4 gap-y-3" style="margin-left: 50px; margin-right: 100px;">
                 <div class="col-span-6 sm:col-span-6"> <label for="modal-form-1" class="form-label ml-1" style="white-space: nowrap;">삭제 처리 하시겠습니까? DB에서 완전히 제거됩니다.</label></div>
             </div>
             <!-- BEGIN: Modal Footer -->
             <div class="modal-footer"> <button id="deleteModalBtn" type="button" data-tw-dismiss="modal"><span class="btn btn-primary w-20 mr-1">확인</span></button> <button type="button" data-tw-dismiss="modal" class="btn btn-outline-secondary w-20">취소</button> </div> <!-- END: Modal Footer -->
         </div>
     </div>
 </div>
 <!-- END: Modal Content -->
...
$(function(){
	if ($("#test").length) {
	    ...
	    var table = new Tabulator("#test", {
			...
			ajaxURL: "/emp/pa/mngmemacnt/awaitingApproval/load",
			ajaxFiltering: false,
			ajaxSorting: false,
			printAsHtml: true,
			printStyled: true,
			...
			columns: [{
			    ...
			    formatter: function(cell, formatterParams) {
			    	...
			    	var icons = $(`
			    	<div class="flex lg:justify-center items-center">
			    		<a class="approval" href="javascript:;" title=${txt}><svg class="text-success" xmlns="http://www.w3.org/2000/svg" width="19" height="19" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round" icon-name="check-square" data-lucide="check-square" class="lucide lucide-check-square w-4 h-4 mr-2"><polyline points="9 11 12 14 22 4"></polyline><path d="M21 12v7a2 2 0 01-2 2H5a2 2 0 01-2-2V5a2 2 0 012-2h11"></path></svg></a>
			    	</div>
				    `);

			    	$(icons).find(".approval").on("click", function() {
			    		const myModal = tailwind.Modal.getInstance(document.querySelector("#approvalModal"));
						myModal.show();
						$("#approvalModalBtn").on("click", function() {
							console.log("클릭");
			    			...
			    			console.log(data);
			    			jQuery.ajax({
			    		        url: '/emp/pa/mngmemacnt/approval',
			    ...
			    formatter: function(cell, formatterParams) {
					...
					$(icons).find(".del").on("click", function() {
						const myModal = tailwind.Modal.getInstance(document.querySelector("#deleteModal"));
						myModal.show();
						$("#deleteModalBtn").on("click", function() {
			    			...
			    			console.log(data);
			    			jQuery.ajax({
			    		        url: '/emp/pa/mngmemacnt/approvalDelete',
			    		        contentType : "application/json;charset=utf-8",
			    		        type: 'post',
			    		        data: JSON.stringify(data),
			    		        dataType : "json",
